...
def main() -> int:
    ...
    parser.add_argument("--skip-navigation", action="store_true", help="Check an assembled archive after each edition's navigation was checked")
    args = parser.parse_args()
    project = tomllib.loads((args.root / "zensical.toml").read_text(encoding="utf-8"))["project"]
    errors = [] if args.skip_navigation else check_navigation(args.root, project)
    if args.site_dir:
